...
export class AppModel implements IAppModel {
  ...
  public async Golive(pathUri?: string) {
    ...
    if (this.IsServerRunning) {
      ...
      this.goLiveEvent.fire({
        runningPort: this.runningPort,
        pathUri: relativePath,
      });
      return this.openBrowser(this.runningPort, relativePath);
    }
    if (pathInfos.isNotOkay) {
      this.showPopUpMsg(
        "Invaild Path in liveShaders.settings.root settings. mm live Server will serve from workspace root",
        true
      );
    }
    ...
    let params = Helper.generateParams(
      pathInfos.rootPath,
      workspacePath,
      () => {
      ...
      if (serverInstance && serverInstance.address) {
        ...
        if (!serverInstance.errorMsg) {
          ...
          this.showPopUpMsg(
            `The default port : ${
              Config.getPort - 1
            } is currently taken, changing port to : ${Config.getPort}.`
          );
          this.Golive(pathUri);
        } else {
          this.showPopUpMsg(
  ...
  changeWorkspaceRoot() {
    setOrChangeWorkspace().then((workspceName) => {
      if (workspceName === undefined) return;
      window.showInformationMessage(
        `Success! '${workspceName}' workspace is now root of Live Shaders`
      );
      // If server is running, Turn off the server.
      if (this.IsServerRunning) this.GoOffline();
    });
  }
  ...
  private haveAnySupportedFile() {
    return new Promise<void>((resolve) => {
      ...
      workspace
        .findFiles(globFormat, "**/node_modules/**", 1)
        .then(async (files) => {
  ...
  private openBrowser(port: number, path: string) {
    ...
    if (useBrowserPreview) {
      ...
      let onError = (err) => {
        this.showPopUpMsg(
          `Server is started at ${this.runningPort} but failed to open in Browser Preview. Got Browser Preview extension installed?`,
          true
        );
    ...
    if (advanceCustomBrowserCmd) {
      ...
      if (
        CustomBrowser &&
        CustomBrowser !== "null" /*For backward capability*/
      ) {
    ...
    try {
      ...
      this.showPopUpMsg(
        `Server is started at ${this.runningPort} but failed to open browser. Try to change the CustomBrowser settings.`,
        true
      );
