...
export function setOrChangeWorkspace() {
  ...
  return window
    .showQuickPick(workspaceNames, {
      placeHolder: "choose workspace for Live Shaders",
      ignoreFocusOut: true,
    })
    .then((workspaceName) => {
      if (workspaceName) {
        return Config.setMutiRootWorkspaceName(workspaceName).then(
          () => workspaceName
        );
      }
    });
  ...
  return new Promise<string>((resolve) => {
    ...
    if (fileUri) {
      ...
      if (selectedWorkspace) {
        return Config.setMutiRootWorkspaceName(selectedWorkspace.name).then(
          () => resolve(selectedWorkspace.uri.fsPath)
        );
      }
    }
    ...
    if (Config.getMutiRootWorkspaceName) {
      ...
      const targetWorkspace = workspaceFolders.find(
        (e) => e.name === Config.getMutiRootWorkspaceName
      );
      if (targetWorkspace) return resolve(targetWorkspace.uri.fsPath);
    ...
    // Show a quick picker
    setOrChangeWorkspace().then((workspaceName) => {
      const workspaceUri = workspaceFolders.find(
