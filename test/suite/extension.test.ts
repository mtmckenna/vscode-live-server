...
suite("Extension Tests", () => {
  ...
  test("should activate", function () {
    ...
    return vscode.extensions
      .getExtension("ritwickdey.LiveServer")
      .activate()
      .then((api) => {
        assert.ok(true);
      });
  ...
  test("should register all live server commands", function () {
    return vscode.commands.getCommands(true).then((commands) => {
      const COMMANDS = [
        ...
        "extension.liveShaders.goOffline",
        "extension.liveShaders.changeWorkspace",
      ];
      const foundLiveServerCommands = commands.filter((value) => {
        return (
          COMMANDS.indexOf(value) >= 0 ||
          value.startsWith("extension.liveShaders.")
        );
      });
