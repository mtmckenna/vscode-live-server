...
const oscServer = new OscServer(
  oscConfig.server.port,
  oscConfig.server.host,
  () => console.log("OSC Server Listening...")
);

const useBrowserExtension = false;
let GET_INJECTED_CODE = () => {
  if (!GET_INJECTED_CODE.cache) {
    GET_INJECTED_CODE.cache = fs.readFileSync(
...
// GET_INJECTED_CODE = fs.readFileSync(
//   path.join(__dirname, "injected.html"),
//   "utf8"
// );
...
LiveServer.start = function (options, callback) {
  ...
  mount.forEach(function (mountRule) {
    ...
    app.use(
      mountRule[0],
      staticServer(mountPath, staticServer, onTagMissedCallback)
    );
  ...
  function handleChange(changePath) {
    ...
    var shaderChange =
      path.extname(changePath) === ".vert" ||
      path.extname(changePath) === ".frag";
  ...
  oscServer.on("message", function (msg, rinfo) {
    console.log("sent OSC message to WS", msg);
    clients.forEach(function (ws) {
